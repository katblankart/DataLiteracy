...
In the following, we go over each chapter to highlight its contribution to Hellerstein’s research article. The structure of Hellerstein contains the following chapters: 
...
In the abstract Hellerstein gives a quick overview on her appear by mentioning the topic, the data, the results and their takeaway


## Introduction

The introduction of Hellerstein is rather on the short sight. She starts it by highlighting a welfare enhancing potential of substituting trade-name drugs with generics, creating relevance for her topic. Next, she elaborates her research question, followed by a short overview of the results and their implications. She finalizes with a structural overview of the rest of her paper.
Other common features of an introduction are missing here. These include among others an overview of the pre-existing research, her contribution to different fields of the literature and an overview of the methodological approach.
...
Hellerstein uses this chapter for two things: to introduce her sample and to show heterogeneities in generic prescription rates in her sample. The first part chapter is thereby built as follows:
...
The data introduction helps readers to understand the sample Hellerstein uses for her analysis; how it originates, what its properties are and how valid is might be for this particular research topic. The remaining part of the chapter emphasises on descriptive differences in generic prescription rates. The heterogeneities Hellerstein focusses on are in particular:
...
In this chapter, Hellerstein clarifies from a theoretical perspective, what factors might determine a physician’s decision to prescribe a generic.
Beyond clarifying the decision situation Hellerstein’s analysis revolved around, the model also serves as a hypothesis for her research question. In the formula, it is clearly defined when the insurance coverage (that closely correlated with the insurance status) nudges a physician towards prescribing a generic.
Last, the model serves as an additional argument, what variables should be considered when empirically modelling the behaviour of physicians.
...
In the results-chapter, Hellerstein again describes her regression model and presents the results. Further she discusses concerns on her estimates and uses other specifications of her model to show her result being robust to them. Some results of the robustness test are also reported.
The chapter starts with a respecification of her empirical application. It is noteworthy, that the model described here deviates from the equation presented in the previous chapter. All estimated coefficients of this single regression are the reported in 3 separate tables:
...
In the text accompanying the tables, she picks numerous coefficients, provides possible explanations for them, and derives their implications.
Next, Hellerstein mentions multiple concerns on her model. She addresses them with different robustness checks. 
These include:

- Use a subsample of patients below age 65
- Use a subsample of physicians with more than 20/30 patients in the overall NAMCS (Results for both of these robustness tests are not reported)
- Use a subsample of observations from states with a special legal system (These results are reported)
- 
With that, Hellerstein not only uses her Results-chapter to report her findings, but also to discuss their implications and limitations. Commonly, this is rather done in a separate Discussion-chapter.


## Conclusion

Hellerstein starts her conclusion with a quick summary of her research article.
...
The appendix the paper consists of two parts:
