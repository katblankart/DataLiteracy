...
<!-- List these three datasets -->

As the confidential data is key to identifying physicians and patients, for the purpose of reproducing Hellerstein's results, we resort to the publicly available data of 1991, that provides these identifiers in two publicly available datasets:

- NAMCS: contains demographic and medical information on the full sample of patients and is mainly used to introduce the analysis population <!--refine: what is analysis population? -->
- NAMCSd: contains information on medications, that were mentioned, prescribed or given to a subsample of patients of the NAMCS data. <!--Include EXACT links to both dataset descriptions-->

The publically available data of NAMCS can be downloaded at the [Centers for Disease Control and Prevention (CDC)](https://www.cdc.gov/nchs/ahcd). The data come in an .exe format. We use the unpacking software 7-zip to convert the files in a .txt format.
...
To process the raw data after we downloaded the raw data from CDC website, a challenge was to structure the raw data set to be able to use the relevant variables in Stata. When we importet the text file into Stata, all data was condensed to one string of characters in a single column. This means that we first need to indicate the different data items (variables) for the individual information (i.e. patient characteristics). Therefore, to be able to use the data, we need to manually split the string into the wanted format, so every information in a separate variable. Which digits in the initial string belong to one data items is defined in the documentation files (also often referred to as code book), provided by the CDC website<!--include link-->. Almost every dataset comes with a codebook, containing useful information on the coding or handling of the data. This can often help and provide the necessary information needed to successfully prepare your data for empirical analysis.

For the reproduction of Hellerstein (1998), we use Stata, creating a .do -file containing the code. It is of importance to develop all necessary steps of your research project to manipulate and analyze data using program files, containing every step you performed with your data. This ensures the reproducibility of your research and enables you to go back to earlier versions of your data preparation and analysis, if necessary.

<!--Highlight the main structure of .do files, i.e. the names of the programs and their purpose -->

<!--Explain the level of aggregation of the analysis dataset --> 

After downloading the NAMCS and NAMCSd we performed the following steps to clean (by excluding non-relevant certain observations), transform and prepare the data for analysis. We concentrate on reproducin all tables and figures originally produced by Hellerstein (1998):

For NAMCS, used for the reproduction of table 1:

<!--make reference to where in the code (e.g. indicating lines) students will find this code--> 

  - Observations of patients, who were not charged for prescription pharmaceuticals, were dropped.
...
After completion of these steps, the analysis dataset included x variables and y observations. The dataset is now ready for statistical analysis (see at the end of the next chapter). Of importance is to ensure that the raw data, the data preparation .do -file and the analysis dataset are stored safely. 

<!-- Include table that shows the variables included in the dataset, use the "describe" command in Stata. -->
