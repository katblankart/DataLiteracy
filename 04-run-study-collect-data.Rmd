...
- **reliability**:  Does the source that you are using provide the data at sufficient quality?
- **potential bias** This arises when the data do not measure what you want to measure. Biases may arise from, for example, changes in the sample of which the data is collected (for example if an institution changes the way data are reported or if the sampled population is changing)
...
The NAMCS data contain data on patient visits to non-federally employed physicians in the United States. For each visit, the physician or other staff members have to complete a one-page survey form containing inter alia patient demographics including the patient’s insurance status, diagnoses, types of health behaviour counselling or medications ordered or provided as well as provider characteristics. As the physicians are randomly chosen and randomly assigned to a two-week reporting period, the NAMCS is able to create a nationally representative sample.
